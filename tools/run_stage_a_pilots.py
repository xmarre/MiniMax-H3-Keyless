...
from minimax_h3_keyless.pilot_runner import run_stage_a_campaign
...
def main() -> int:
    ...
    for block_index in (0, 25, 49):
        artifact_request.assert_available(block_index, plan.stages[-1].stage)

    teacher = load_pinned_bf16_teacher(args.teacher)
    result = run_stage_a_campaign(
        teacher.pilot_blocks,
        capture_set,
        device=args.device,
    ...
    payload = {
        ...
        "run_id": args.run_id,
        "code_commit": args.code_commit,
        "teacher_sha256": teacher.signature_report.teacher_sha256,
        ...
        "block_artifacts": {
            str(index): asdict(block.artifact) if block.artifact is not None else None
    ...
    print(f"Stage-A campaign gate passed: {result.gate.passed}")
